...
def find_closest(verts, x, y, z):
    """find the closest point to [x, y, z] from verts using the l2
    simmilarity."""
...
if __name__ == '__main__':
    ...
    features = torch.ones((len(verts), 3)).to(verts.device)
    features[:, 0] = 0
    points = Pointclouds(points=[verts], features=[features])

    T_Z, AZIM, ELEV, AT, UP, AXIS = _view_definitions()

    base = f'{app_path}/data/test_injected/trainval/images'

    for angle in [0, 20, 45]:

        folder_path = f'{base}/{angle}_up_{UP}_axis_{AXIS}'
