...
def test_formula(verts):
    ...
    for angle in [10 * i for i in range(1, 19)]:
        ...
        assert z.item() == 0, "formula for calculating verts position over the image is wrong"
...
if __name__ == "__main__":
    ...
    for angle in [0, 20, 45]:
        ...
        folder_path = f"{app_path}/data/test_injected/trainval/images/{angle}_up_{UP}_axis_{AXIS}"
        ...
        points_render_image, pixels_to_highlight = render_extreme_points(mesh=mesh, cameras=cameras, axis=AXIS, up_vec=UP, angle=angle)


        vertex_clip = (vertex @ R_T[0]) @ P[0]
        vertex_ndc = vertex_clip[:, :3] / vertex_clip[:, 3:] 

        x_pixel_int, y_pixel_int = pixels_to_image_coordinates(xs=vertex_ndc[:, 0], ys=vertex_ndc[:, 1])
        ...
        Image.fromarray((image_for_my_projection).astype(np.uint8)).save(f"{folder_path}/hand_craft_extreme_point.png")

        image_for_my_projection = np.zeros((IMAGE_SIZE, IMAGE_SIZE, 3)).astype(np.uint8)
        image_for_my_projection[y_pixel_int, x_pixel_int] = paint_color
        Image.fromarray((image_for_my_projection).astype(np.uint8)).save(f"{folder_path}/my_projection.png")

        image_for_my_projection[pixels_to_highlight[:, 0], pixels_to_highlight[:, 1]] = np.array([0, 0, 255])  # Blue

        torch_points_render_image = render_points_only(points, cameras)



        im_for_rand = Image.open(f"{app_path}/data/test_injected/trainval/images/P0005__1024__0___0.png")
        ...
        Image.fromarray((points_render_image * 255).astype(np.uint8)).save(f"{folder_path}/experiment_{AXIS}.png")
        Image.fromarray(end_im).save(f"{folder_path}/pres_dota.png")
        Image.fromarray((torch_points_render_image * 255).astype(np.uint8)).save(f"{folder_path}/point_cloud_theirs.png")
        Image.fromarray((.5 * (torch_points_render_image * 255) + .5 * end_im.astype(np.float32)).astype(np.uint8)).save(f"{folder_path}/theirs_over_obj.png")
        Image.fromarray((.5 * image_for_my_projection + .5 * end_im.astype(np.float32)).astype(np.uint8)).save(f"{folder_path}/my_over-obj.png")
        Image.fromarray((.5 * (torch_points_render_image * 255) + .5 * image_for_my_projection.astype(np.float32)).astype(np.uint8)).save(f"{folder_path}/my_proj_over_camera_proj.png")

        
        print(f"done {angle}, visualizations can be found at - {folder_path}", end="\n\n")
